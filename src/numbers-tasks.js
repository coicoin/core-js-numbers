 ...
 * function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  // calc length of vectors
  const av = Math.sqrt(x1 ** 2 + y1 ** 2);
  const bv = Math.sqrt(x2 ** 2 + y2 ** 2);
  // calc cos of angle
  const cos = dotProduct / (av * bv);
  // return radians with arcos
...
function roundToPowerOfTen(num, pow) {
  const decimalPlaces = 10 ** pow;
  return (num / decimalPlaces).toFixed() * decimalPlaces;
}
...
function toNumber(value, def) {
  ...
  return !Number.isNaN(converted) ? converted : def;
}
